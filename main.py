...
def traink(train_dev_data, train_index, dev_index, model, criterion, optimizer, config):
    ...
    train_labels = torch.from_numpy(np.array(train_dev_labels, dtype=int64)[train_index]).to(config.device)
    dev_labels   = torch.from_numpy(np.array(train_dev_labels, dtype=int64)[dev_index]).to(config.device)
    ...
    for epoch in trange(1, config.epoch+1):
        if epoch == config.change_lr_epoch or epoch == 10:
            current_lr *= 0.1
        ...
        for step, (data, label) in enumerate(tbar):
            ...
            desc = 'Training: Epoch : %d/%d, Iter : %d/%d,  Loss: %.4f, Current lr: %.4f' % \
                   (epoch, config.epoch, step + 1, len(train_loader), loss.item(), current_lr)
